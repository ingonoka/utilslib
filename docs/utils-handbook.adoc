...
=== Base64
Fast BASE64 encoding functions based on lookup tables.
...
- "PT10H"     -- parses as "10 hours" (one hour is 3600 seconds)
- "P2D"       -- parses as "2 days" (one day is 24 hours or 86,400 seconds)
...
- The buffer capacity will be increased by multiples of the initial buffer size.  For example, writing 129 bytes to a buffer that is 128 bytes long will result in a buffer with a capacity of 256 bytes.
...
----
- Reading and writing of Int and Long takes four and eight bytes and is ordered in big endian notation
